...
d <- tulips
str(d)
#making a dummy vars for categorical variable. For bed a, it will be absorbed into the intercept, and then the coefficients for each dummy variable will contain contrasts with bed a.
...
model1 <- quap(
  alist(
    ...
    mu <- a + Bb*bed_b + Bc*bed_c + Bw*water + Bs*shade + Bws*water*shade,
    a~dnorm(0, 100),
    c(Bb, Bc, Bw, Bs, Bws)~dnorm(0, 100),
    sigma~dunif(0, 100)
  ),
  start=list(a = 130, Bb = 0, Bc = 0, Bw = 0, Bs = 0, Bws = 0, sigma = 90),
  data = d
)
precis(model1)
```
...
library(tidyverse)
data(rugged)
d <- rugged[complete.cases(rugged$rgdppc_2000),] %>% filter(country != "Seychelles") %>% mutate(log_gdp = log(rgdppc_2000), rugged_c = rugged - mean(rugged))

model1 <- quap(
  alist(log_gdp~dnorm(mean, sigma),
        mean <- a + bA*cont_africa + bR*rugged_c + bAR*cont_africa*rugged_c,
        a ~ dnorm(0, 100),
...
library(tidyverse)
data(nettle)
d <- nettle %>% mutate(lang.per.cap = num.lang/k.pop)
d <- d[complete.cases(d$lang.per.cap), ] %>% mutate(log.lpc = log(lang.per.cap), mgs = mean.growing.season - mean(mean.growing.season), area = log(area))
